...
public class CalculationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (bundle.getString("Data")) {
            case "bricks":
                if (bundle != null){
                    count = (int) Math.round(bundle.getInt("wallSquare") / bundle.getInt("brickSquare"));
                    costAll = bundle.getInt("cost");
                    output.setText("Для строительства нужно " + count + " плиток\nИх стоимость " + costAll + " монет");
                }
                break;
            case "wallpapers":
                if (bundle != null) {
        ...
        public void onClick(View view) {
            ...
            switch (view.getId()) {
                case R.id.buttonBack:
                    ...
                    if (bundle.getString("Data") == "wallpapers") {
                    ...
                    if (bundle.getString("Data") == "bricks") {
                        intent = new Intent(getApplicationContext(), WallbricksActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP); // удаление всех активностей кроме той на которую происходит переход
                        break;
                    }
